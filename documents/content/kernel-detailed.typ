...
#text()[#h(2em)] 通过条件编译（如 `#ifdef` RISCV）自动适配不同架构下的页表实现，保证接口一致性与可移植性。
...
#text()[#h(2em)] 对于具体的文件系统，只需要移植自己的超级块结构体，并完善同样的函数实现。如ext4的超级块：
...
#text()[#h(2em)] 该结构体与linux的shm_segment结构体兼容，包含了共享内存段的ID、键值、大小、权限标志等信息。与原本的linux结构体不同，它维护了一个附加地址列表，用于跟踪所有附加到该共享内存段的虚拟地址并存储附加地址的唯一tid，该方法可以支持多个进程附加到同一个共享内存段，并使用键值区别不同页表上的附加值。
...
#text()[#h(2em)] 这些方法实现了共享内存段的创建、删除、附加和分离等基本操作，符合POSIX标准的共享内存API规范。
