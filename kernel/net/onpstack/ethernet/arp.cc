...
#include "ip/ip.hh"
#include "klib.hh"  // 包含 memset, memcpy 等函数
#include "printer.hh"  // 包含 printf 函数声明

#if SUPPORT_ETHERNET
#include "ethernet/arp_frame.hh"
...
static INT ipv4_to_mac(PST_NETIF pstNetif, UINT unDstArpIPAddr, UCHAR ubaMacAddr[ETH_MAC_ADDR_LEN])
{
	...
	//* 是否命中最近刚读取过的条目
	os_enter_critical();
	if (unDstArpIPAddr == pstcbArp->staEntry[(int)pstcbArp->bLastReadEntryIdx].unIPAddr)
	{
		memcpy(ubaMacAddr, pstcbArp->staEntry[(int)pstcbArp->bLastReadEntryIdx].ubaMacAddr, ETH_MAC_ADDR_LEN);
		pstcbArp->staEntry[(int)pstcbArp->bLastReadEntryIdx].unUpdateTime = os_get_system_secs();
		os_exit_critical();
		return 0;
	}
	os_exit_critical();
...
INT arp_send_request_ethii_ipv4(PST_NETIF pstNetif, UINT unSrcIPAddr, UINT unDstArpIPAddr, EN_ONPSERR *penErr)
{
    ...
    //* 发送查询报文
    static UCHAR ubaBroadcastMac[ETH_MAC_ADDR_LEN] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    INT nRtnVal = pstNetif->pfunSend(pstNetif, ARP, sBufListHead, ubaBroadcastMac, penErr); 
