            ...
            if (TCPOPT_SACKINFO == enType)
            {
                ...
                if (bWriteIdx > 1 /*2*/)
                {
                    ...
                    if (uint_before(pstLink->stcbSend.staSack[0].unLeft, pstLink->stLocal.unSeqNum) ||
                        (!uint_after(pstLink->stcbSend.staSack[0].unRight, pstLink->stcbSend.staSack[1].unRight) && !uint_before(pstLink->stcbSend.staSack[0].unLeft, pstLink->stcbSend.staSack[1].unLeft)))
                    {
                        bWriteIdx--;
