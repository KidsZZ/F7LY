...
static inline uint32_t ext4_sb_get_block_size(struct ext4_sblock *s) { return 1024 << to_le32(s->log_block_size); }
...
static inline uint16_t ext4_sb_get_desc_size(struct ext4_sblock *s) {
    ...
    return size < EXT4_MIN_BLOCK_GROUP_DESCRIPTOR_SIZE ? EXT4_MIN_BLOCK_GROUP_DESCRIPTOR_SIZE : size;
}
...
static inline bool ext4_sb_check_flag(struct ext4_sblock *s, uint32_t v) { return to_le32(s->flags) & v; }
 ...
 * @param   s superblock descriptor
 * @return  flex bg size*/
static inline uint32_t ext4_sb_flex_bg_size(struct ext4_sblock *s) { return 1 << to_le32(s->log_groups_per_flex); }

/**@brief   Return first meta block group id.
 * @param   s superblock descriptor
 * @return  first meta_bg id */
static inline uint32_t ext4_sb_first_meta_bg(struct ext4_sblock *s) { return to_le32(s->first_meta_bg); }
