...
int platform_init(SDMMC_T *pSDMMC) {
  ...
  while (pSDMMC->CTRL &
         (MCI_CTRL_RESET | MCI_CTRL_FIFO_RESET | MCI_CTRL_DMA_RESET)) {  // 等待复位完成
    printfPink("ctrl:%p\n", pSDMMC->CTRL);
  }
...
uint32 wait_for_sdio_irq(SDMMC_T *pSDMMC) {
  ...
  while (1) {
    ...
    // printf("rintst: %p\n", rintst);
    if (rintst & 0xffff0004) {  // 检查是否有命令完成或错误中断
      break;
    }
  }
...
uint32 SD_Send_Command(SDMMC_T *pSDMMC, uint32 cmd, uint32 arg) {
  ...
  /* 如果是数据传输命令，则启用数据传输中断 */
  if (cmd & SDIO_CMD_DATA) {  // 检查命令是否包含数据传输
...
int SD_SendCmd(SDMMC_T *pSDMMC, uint32 cmd, uint32 arg) {
  ...
  while (--tmo && (pSDMMC->CMD & MCI_CMD_START)) {  // 等待CIU接受命令(START位被清除)
    if (tmo & 1) {
      delay = 50;
    } else {
      delay = 18000;
