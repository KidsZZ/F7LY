 ...
 */
static inline uint32_t ext4_dir_en_get_inode(struct ext4_dir_en *de) { return to_le32(de->inode); }

/**@brief Set i-node number to directory entry.
 * @param de Directory entry
 * @param inode I-node number
 */
static inline void ext4_dir_en_set_inode(struct ext4_dir_en *de, uint32_t inode) { de->inode = to_le32(inode); }
 ...
 * @return Entry length
 */
static inline uint16_t ext4_dir_en_get_entry_len(struct ext4_dir_en *de) { return to_le16(de->entry_len); }

/**@brief Set directory entry length.
 * @param de     Directory entry
 * @param l Entry length
 */
static inline void ext4_dir_en_set_entry_len(struct ext4_dir_en *de, uint16_t l) { de->entry_len = to_le16(l); }
...
static inline uint16_t ext4_dir_en_get_name_len(struct ext4_sblock *sb, struct ext4_dir_en *de) {
    ...
    if ((ext4_get32(sb, rev_level) == 0) && (ext4_get32(sb, minor_rev_level) < 5))
