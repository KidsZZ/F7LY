...
#define EXT4_EXTENT_FIRST(header) ((struct ext4_extent *)(((char *)(header)) + sizeof(struct ext4_extent_header)))
...
#define EXT_LAST_EXTENT(__hdr__) (EXT_FIRST_EXTENT((__hdr__)) + to_le16((__hdr__)->entries_count) - 1)
#define EXT_LAST_INDEX(__hdr__) (EXT_FIRST_INDEX((__hdr__)) + to_le16((__hdr__)->entries_count) - 1)
#define EXT_MAX_EXTENT(__hdr__) (EXT_FIRST_EXTENT((__hdr__)) + to_le16((__hdr__)->max_entries_count) - 1)
#define EXT_MAX_INDEX(__hdr__) (EXT_FIRST_INDEX((__hdr__)) + to_le16((__hdr__)->max_entries_count) - 1)

#define EXT4_EXTENT_TAIL_OFFSET(hdr) \
    (sizeof(struct ext4_extent_header) + (sizeof(struct ext4_extent) * to_le16((hdr)->max_entries_count)))
...
static inline uint32_t ext4_extent_get_first_block(struct ext4_extent *extent) { return to_le32(extent->first_block); }
...
void ext4_extent_tree_init(struct ext4_inode_ref *inode_ref)
{
    ...
    uint16_t max_entries =
        (EXT4_INODE_BLOCKS * sizeof(uint32_t) - sizeof(struct ext4_extent_header)) / sizeof(struct ext4_extent);
...
{
    return (to_le16(ext->block_count) <= EXT_INIT_MAX_LEN ? to_le16(ext->block_count)
...
static void ext4_ext_mark_unwritten(struct ext4_extent *ext) { ext->block_count |= to_le16(EXT_INIT_MAX_LEN); }
...
static ext4_fsblk_t ext4_new_meta_blocks(struct ext4_inode_ref *inode_ref, ext4_fsblk_t goal, uint32_t flags __unused,
...
static void ext4_ext_free_blocks(struct ext4_inode_ref *inode_ref, ext4_fsblk_t block, uint32_t count,
...
static ext4_fsblk_t ext4_ext_find_goal(struct ext4_inode_ref *inode_ref, struct ext4_extent_path *path,
                                       ext4_lblk_t block)
{
    if (path)
    {
 ...
 */
static ext4_fsblk_t ext4_ext_new_meta_block(struct ext4_inode_ref *inode_ref, struct ext4_extent_path *path,
                                            struct ext4_extent *ex, int *err, uint32_t flags)
{
...
static uint32_t ext4_ext_block_csum(struct ext4_inode_ref *inode_ref, struct ext4_extent_header *eh)
{
    uint32_t checksum = 0;
...
static void ext4_extent_block_csum_set(struct ext4_inode_ref *inode_ref __unused, struct ext4_extent_header *eh)
{
...
static void ext4_ext_drop_refs(struct ext4_inode_ref *inode_ref, struct ext4_extent_path *path, bool keep_other)
{
    int32_t depth, i;
 ...
 * is correct or not.
 */
static int ext4_ext_check(struct ext4_inode_ref *inode_ref, struct ext4_extent_header *eh, uint16_t depth,
...
static int read_extent_tree_block(struct ext4_inode_ref *inode_ref, ext4_fsblk_t pblk, int32_t depth,
                                  struct ext4_block *bh, uint32_t flags __unused)
{
    int err;
...
static int ext4_find_extent(struct ext4_inode_ref *inode_ref, ext4_lblk_t block, struct ext4_extent_path **orig_path,
                            uint32_t flags)
{
    ...
    if (!path)
    {
        ...
        path = (ext4_extent_path *)ext4_calloc(1, sizeof(struct ext4_extent_path) * (path_depth + 1));
        if (!path)
            return ENOMEM;
...
static void ext4_ext_init_header(struct ext4_inode_ref *inode_ref, struct ext4_extent_header *eh, int32_t depth)
{
    eh->entries_count = 0;
...
static int ext4_ext_insert_index(struct ext4_inode_ref *inode_ref, struct ext4_extent_path *path, int at,
                                 ext4_lblk_t insert_index, ext4_fsblk_t insert_block, bool set_to_ix)
{
...
static int ext4_ext_split_node(struct ext4_inode_ref *inode_ref, struct ext4_extent_path *path, int at,
                               struct ext4_extent *newext, struct ext4_extent_path *npath, bool *ins_right_leaf)
{
    int i, npath_at, ret;
    ...
    ret = ext4_ext_insert_index(inode_ref, path, at - 1, insert_index, npath[0].block.lb_id, *ins_right_leaf);
 ...
 */
static int ext4_ext_correct_indexes(struct ext4_inode_ref *inode_ref, struct ext4_extent_path *path)
{
    struct ext4_extent_header *eh;
...
{
    if (ext4_ext_pblock(ex2) + ext4_ext_get_actual_len(ex2) != ext4_ext_pblock(ex1))
        return 0;
...
{
    if (ext4_ext_pblock(ex1) + ext4_ext_get_actual_len(ex1) != ext4_ext_pblock(ex2))
        return 0;
...
static int ext4_ext_insert_leaf(struct ext4_inode_ref *inode_ref, struct ext4_extent_path *path, int at,
                                struct ext4_extent *newext, int flags, bool *need_split)
{
    struct ext4_extent_path *curp = path + at;
...
static inline void ext4_ext_replace_path(struct ext4_inode_ref *inode_ref, struct ext4_extent_path *path,
                                         struct ext4_extent_path *newpath, int at)
{
...
again:
    ...
    if (ret == EIO && need_split == true)
    {
        ...
        if (level > 0)
        {
            npath = (ext4_extent_path *)ext4_calloc(1, sizeof(struct ext4_extent_path) * (level));
            if (!npath)
            {
                ret = ENOMEM;
                goto out;
...
static void ext4_ext_remove_blocks(struct ext4_inode_ref *inode_ref, struct ext4_extent *ex, ext4_lblk_t from,
                                   ext4_lblk_t to)
{
...
static int ext4_ext_remove_idx(struct ext4_inode_ref *inode_ref, struct ext4_extent_path *path, int32_t depth)
{
    int err = EOK;
    int32_t i = depth;
    ext4_fsblk_t leaf;
...
static int ext4_ext_remove_leaf(struct ext4_inode_ref *inode_ref, struct ext4_extent_path *path, ext4_lblk_t from,
                                ext4_lblk_t to)
{
...
int ext4_extent_remove_space(struct ext4_inode_ref *inode_ref, ext4_lblk_t from, ext4_lblk_t to)
{
    ...
    in_range =
        IN_RANGE(from, to_le32(path[depth].extent->first_block), ext4_ext_get_actual_len(path[depth].extent));
    ...
    if ((to_le32(path[depth].extent->first_block) < from) &&
        (to < to_le32(path[depth].extent->first_block) + ext4_ext_get_actual_len(path[depth].extent) - 1))
    {
    ...
    while (i >= 0)
    {
        ...
        if (ext4_ext_more_to_rm(path + i, to))
        {
            ...
            ret = read_extent_tree_block(inode_ref, ext4_idx_pblock(path[i].index), depth - i - 1, &bh, 0);
            if (ret != EOK)
                goto out;
...
static int ext4_ext_split_extent_at(struct ext4_inode_ref *inode_ref, struct ext4_extent_path **ppath,
...
static int ext4_ext_convert_to_initialized(struct ext4_inode_ref *inode_ref, struct ext4_extent_path **ppath,
                                           ext4_lblk_t split, uint32_t blocks)
{
    ...
    else
    {
        ...
        err = ext4_ext_split_extent_at(inode_ref, ppath, split + blocks, EXT4_EXT_MARK_UNWRIT1 | EXT4_EXT_MARK_UNWRIT2);
        if (err == EOK)
        {
            err = ext4_ext_split_extent_at(inode_ref, ppath, split, EXT4_EXT_MARK_UNWRIT1);
        }
...
static int ext4_ext_zero_unwritten_range(struct ext4_inode_ref *inode_ref, ext4_fsblk_t block, uint32_t blocks_count)
{
    int err = EOK;
    uint32_t i;
    uint32_t block_size = ext4_sb_get_block_size(&inode_ref->fs->sb);
