...
#define BIND_SYSCALL(sysname)                                       \
    ...
    uint64 SyscallHandler::sys_openat()
    {
        ...
        if (abs_pathname.find("/proc/self/fd/") == 0)
        {
            ...
            if (!target_file)
            {
                printfRed("[SyscallHandler::sys_openat] 无效的文件描述符: %d\n", target_fd);
                return SYS_EBADF;
            }

            // 对于 /proc/self/fd/ 路径，应该直接复制文件描述符而不是重新打开
            // 这相当于 dup() 操作，但需要处理某些标志
            printfCyan("[SyscallHandler::sys_openat] duplicating fd %d for /proc/self/fd/%d\n", target_fd, target_fd);
            ...
            if (flags & O_TRUNC)
            {
                ...
                if (target_file->lwext4_file_struct.fsize > 0)
                {
                    ...
                    printfCyan("[SyscallHandler::sys_openat] File successfully truncated to 0 bytes\n");
                }
            }
            ...
            {
                // 对于已存在的文件描述符，这些标志不适用
                printfYellow("[SyscallHandler::sys_openat] ignoring O_CREAT/O_EXCL flags for /proc/self/fd/ path\n");
            ...
            if (new_fd < 0)
            {
                printfRed("[SyscallHandler::sys_openat] Error allocating fd for /proc/self/fd/ path\n");
                return SYS_EMFILE;
            }
...
// while (1);
    ...
    uint64 SyscallHandler::sys_ftruncate()
    {
        ...
        {
            printfRed("[sys_ftruncate] Cannot truncate a socket file: %d\n", fd);
            return SYS_EINVAL; // 无效的文件类型，不能对socket文件进行截
        }
        if (length < 0)
        {
    ...
    uint64 SyscallHandler::sys_fchownat()
    {
        ...
        if (pathname.empty())
        {
            ...
            {
                uint32_t fuid = 0, fgid = 0;
                int r0 = vfs_owner_get(target_path, fuid, fgid, /*follow_symlinks*/ true);
                if (r0 != EOK)
                    return -EACCES;
                if (fuid != p->get_euid())
                    return SYS_EPERM;
        ...
        {
            uint32_t fuid = 0, fgid = 0;
            int r0 = vfs_owner_get(abs_pathname, fuid, fgid, /*follow_symlinks*/ !(flags & AT_SYMLINK_NOFOLLOW));
            if (r0 != EOK)
                return -EACCES;
            if (fuid != p->get_euid())
                return SYS_EPERM;
    ...
    uint64 SyscallHandler::sys_fchown()
    {
        ...
        {
            uint32_t fuid = 0, fgid = 0;
            int r0 = vfs_owner_get(path, fuid, fgid, /*follow_symlinks*/ true);
            if (r0 != EOK)
                return -EACCES;
            if (fuid != p->get_euid())
                return -EPERM;
            if (!(gid == (int)p->get_gid() || gid == (int)p->get_egid()))
                return -EPERM;
        }
