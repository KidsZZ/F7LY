...
namespace syscall
{
    ...
    // // Use constexpr int for negative error codes instead of enum
    // constexpr int EPERM = -1;           // 操作不允许（无权限）
    // constexpr int ENOENT = -2;          // 文件或目录不存在
    // constexpr int ESRCH = -3;           // 进程不存在
    // constexpr int EINTR = -4;           // 系统调用被信号中断
    // constexpr int EIO = -5;             // 输入/输出错误（底层硬件或设备故障）
    // constexpr int ENXIO = -6;           // 设备或地址不存在
    // constexpr int E2BIG = -7;           // 参数列表过长（如 execve 的参数）
    // constexpr int ENOEXEC = -8;         // 可执行文件格式错误
    // constexpr int EBADF = -9;           // 错误的文件描述符（未打开或无效）
    // constexpr int ECHILD = -10;         // 无子进程（如 waitpid 无目标）
    // constexpr int EAGAIN = -11;         // 资源暂时不可用（非阻塞操作未就绪），等同于 EWOULDBLOCK
    // constexpr int ENOMEM = -12;         // 内存不足
    // constexpr int EACCES = -13;         // 权限不足（文件访问被拒绝）
    // constexpr int EFAULT = -14;         // 错误的地址（用户空间指针无效）
    // constexpr int ENOTBLK = -15;        // 需要块设备（如对字符设备执行块操作）
    // constexpr int EBUSY = -16;          // 设备或资源忙（如文件被锁定）
    // constexpr int EEXIST = -17;         // 文件已存在（如创建已存在的文件）
    // constexpr int EXDEV = -18;          // 跨设备链接（不允许跨文件系统硬链接）
    // constexpr int ENODEV = -19;         // 设备不存在
    // constexpr int ENOTDIR = -20;        // 不是目录（期望目录但提供的是文件）
    // constexpr int EISDIR = -21;         // 是目录（期望文件但提供的是目录）
    // constexpr int EINVAL = -22;         // 无效参数（如错误的标志值）
    // constexpr int ENFILE = -23;         // 系统文件表溢出（全局文件描述符耗尽）
    // constexpr int EMFILE = -24;         // 进程打开文件数超出限制
    // constexpr int ENOTTY = -25;         // 不是终端设备（如对非终端调用 ioctl）
    // constexpr int ETXTBSY = -26;        // 文本文件忙（如正在执行的共享库被修改）
    // constexpr int EFBIG = -27;          // 文件过大（超出文件大小限制）
    // constexpr int ENOSPC = -28;         // 设备空间不足（如磁盘写满）
    // constexpr int ESPIPE = -29;         // 非法寻址（如对管道调用 lseek）
    // constexpr int EROFS = -30;          // 只读文件系统（尝试修改只读挂载的文件系统）
    // constexpr int EMLINK = -31;         // 链接数过多（文件系统限制）
    // constexpr int EPIPE = -32;          // 管道破裂（写入无读取端的管道）
    // constexpr int EDOM = -33;           // 数学参数超出函数定义域
    // constexpr int ERANGE = -34;         // 数学结果不可表示（如溢出）
    // constexpr int EDEADLK = -35;        // 资源死锁可能发生（如线程锁顺序问题）
    // constexpr int ENAMETOOLONG = -36;   // 文件名过长（超出文件系统限制）
    // constexpr int ENOLCK = -37;         // 无可用记录锁（文件锁资源耗尽）
    // constexpr int ENOSYS = -38;         // 无效的系统调用号（如不存在的 syscall）
    // constexpr int ENOTEMPTY = -39;      // 目录非空（如删除非空目录）
    // constexpr int ELOOP = -40;          // 符号链接嵌套过深（可能形成环路）
    // constexpr int ENOTSOCK = -88;       // 对非套接字执行套接字操作
    // constexpr int EMSGSIZE = -90;       // 发送信息超过一次message最大内容
    // constexpr int ENOPROTOOPT = -92;
    // constexpr int EPROTONOSUPPORT = -93;// EPROTONOSUPPORT表示不支持所选的套接字协议
    // constexpr int EOPNOTSUPP = -95;     // 操作不支持（如对普通文件调用套接字操作）
    // constexpr int EAFNOSUPPORT = -97;   // address family 不支持
    // constexpr int EADDRINUSE = -98;     // 套接字地址已在使用中（如端口被占用）
    // constexpr int EADDRNOTAVAIL = -99;  // 地址不可用（如绑定到不存在的 IP）
    // constexpr int ECONNABORTED = -103;
    // constexpr int ECONNRESET = -104;    // 连接被重置（对端强制关闭）
    // constexpr int EISCONN = -106;       // 传输端点已连接（如重复调用 connect）
    // constexpr int ENOTCONN = -107;      // 套接字未连接（如未 connect 就 send）
    // constexpr int ETIMEDOUT = -110;     // 操作超时（如网络请求未在指定时间内响应）
    // constexpr int ECONNREFUSED = -111;  // 连接被拒绝（对端无监听服务）
    // constexpr int EINPROGRESS = -115;   // 套接字为非阻塞模式且连接无法立即完成
    // constexpr int ERESTARTSYS = -512;   // 内核自动重启系统调用
}