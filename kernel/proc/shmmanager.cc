...
namespace proc
{
	...
	void *ShmManager::shmgetat( uint key, uint num )
	{
		...
		{
			shm = mem::k_vmm.allocshm( pg, shm, shm - num * PGSIZE, pcb->_sz, phyaddr );
			if ( shm == 0 )
			{
				Shmtabs[ key ]._lock.release();
		...
		else
		{
			...
			if ( ( shm = mem::k_vmm.mapshm( pg, shm, shm - num * PGSIZE, pcb->_sz, phyaddr ) ) == 0 )
			{
				Shmtabs[ key ]._lock.release();
				return reinterpret_cast< void* >( -1 );
			}
	...
	int ShmManager::shmrm( uint key )
	{
		...
		{
			mem::k_pmm.free_page( shmem->phyaddr[ i ] );
		}
		shmem->refcnt = 0;
		return 0;
	}

	int ShmManager::shmrelease( mem::PageTable &pt, uint64 shm, uint keymask )
	{
		Shmtabs[ keymask ]._lock.acquire();
		mem::k_vmm.deallocshm( pt, shm, TRAPFRAME - 64 * 2 * PGSIZE);
		for ( int k = 0; k < 8; k++ )
		{
