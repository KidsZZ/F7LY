...
F7LY-OS是C++编写的支持RISC-V与LoongArch双架构的宏内核模块化操作系统，由来自武汉大学的三名队员使用C++从0参考xv6并改编、增加功能构建而成。使用分体的硬件抽象层实现架构无关的内核核心功能，进行多阶段启动，提供统一的进程管理，细粒度的内存资源动态管理，拥有统一接口的虚拟文件系统，可使用C++的多态特性支持fat32与ext4，以及基础的信号机制。开发过程的全部commit记录请参见原仓库地址#link("https://github.com/KidsZZ/F7LY")[KidsZZ/F7LY]。
...
- *内存管理*：对内存布局进行了重大调整，修改了堆内存及物理内存的范围，并添加了共享内存区域。通过使用独立的程序内存管理器的方法重新规划进程的地址空间，使得进程内存区域的管理更加科学合理。
